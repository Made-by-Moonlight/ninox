...
mod tests {
    ...
    fn switch_to_split_panel_resizes_terminal_to_fit_narrower_area() {
        ...
        // Simulate a PTY already sized for the full-width Terminal panel.
        m.terminals.insert(
            "s1".into(),
        ...
        let term = m2.terminals.get("s1").expect("terminal state kept across panel switch");
        assert!(
            term.term.grid().columns() < full_width_cols as usize,
            "switching to Split must reflow the terminal grid to fit alongside the info panel, not leave it clipped at the old width"
        );
